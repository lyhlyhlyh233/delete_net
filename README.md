...
核心函数直接接收调用方的 `std::string`：

```cpp
DWORD error = RemoveDevice(deviceId);
```

内部直接调用 `SetupDiOpenDeviceInfoA`，常见 PCI、USB 和 ROOT 设备实例 ID 都可
直接传入，不需要先转换为 `std::wstring`。
